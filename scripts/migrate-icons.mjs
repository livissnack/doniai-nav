...
function migrateContent(content) {
  ...
  // <i :class="['fas', menuOpen ? 'fa-times' : 'fa-bars']" ...>
  next = next.replace(
    /<i\s+:class="\['(fa[bst]?|fa)',\s*([^?]+)\?\s*'(fa-[^']+)'\s*:\s*'(fa-[^']+)'\]"\s*([^/>]*)\/>/g,
    (_, pack, cond, a, b, attrs) => {
      const packVal = packFromClass(pack)
      const packAttr = packVal === 'solid' ? '' : ` pack="${packVal}"`
      const nA = a.replace(/^fa-/, '')
      const nB = b.replace(/^fa-/, '')
      return `<AppIcon :name="${cond} ? '${nA}' : '${nB}'"${packAttr} ${attrs.trim()} />`
    },
  )

  // <i :class="['fas', x ? 'fa-a' : 'fa-b']"></i> with closing tag
  next = next.replace(
  ...
  // <i class="fas" :class="item.icon"></i>  (tools)
  next = next.replace(
  ...
  next = next.replace(
    ...
    (match, expr, attrs) => {
      ...
      if (!expr.includes('fa-') && !expr.includes("'fas") && !expr.includes('"fas')) return match
      return `<AppIcon :icon-class="${expr}" ${attrs.trim()} />`
    },
  )

  // icon: 'fa-fire-alt' -> icon: 'fire-alt' in data
  ...
  next = next.replace(/icon:\s*'(fas|far|fab)\s+(fa-[^']+)'/g, (_, pack, icon) => {
    ...
    return p === 'solid' ? `icon: '${icon.replace(/^fa-/, '')}'` : `icon: '${icon.replace(/^fa-/, '')}', iconPack: '${p}'`
  })
  next = next.replace(/icon:\s*'fas\s+fa-([^']+)'/g, "icon: '$1'")
